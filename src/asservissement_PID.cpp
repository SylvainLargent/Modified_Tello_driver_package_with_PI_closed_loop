...
int main(int argc, char ** argv){
    ros::init(argc, argv, "assservissement_PID");
    //Voliere de l'U2IS
    Env environment = Env(-1.5, 0.6, -1.4, 1.8, -0.1, 2, 0.1, 0.1, 0.1);
...
//Defining the obstacles of the scene
    vector<vector<double>> list_rectangles =
        {   
            ...
            {-1.65, -0.75, -0.1, 1.9, 1.1, 3}      //Obstacle virtuel
        };
...
//Planning    
    ...
    Point3 * s_goal = new Point3(coord_cible.pose.position.x -0.065, coord_cible.pose.position.y - 0.2, (coord_cible.pose.position.z + 0.3));


    //Dans le cas où step_len =  0.25 
    double step_len = 0.25;
    double Kp = 1.6;                       //Coefficient proportionnel du PID
    ...
    double precision_landing = 0.1;       //Precision au niveau de l'arrivée
...
//Giving out, arena necessary info !
    ...
    arena_boundaries.open("/home/student/Desktop/Sylvain/3D_rrt_plot_cpp/csv_files/boundaries.txt");
    ...
    start_destination.open("/home/student/Desktop/Sylvain/3D_rrt_plot_cpp/csv_files/start_destination.txt");
...
//Plotting rectangle obstacles thanks to their vertices
    ...
    all_boundaries_stream.open("/home/student/Desktop/Sylvain/3D_rrt_plot_cpp/csv_files/obstacles.txt");
...
//Plotting path trajectory
    ...
    path_stream.open("/home/student/Desktop/Sylvain/3D_rrt_plot_cpp/csv_files/trajectory.txt");
    if(!path.empty()){
...
//Plotting tree 
    ...
    tree_stream.open("/home/student/Desktop/Sylvain/3D_rrt_plot_cpp/csv_files/tree.txt");
    if(!tree.empty()){
        ...
        while (ros::ok()) {
            ...
            if(distance(position, *s_goal) < precision){
                ...
                while(distance(position, *s_goal) > precision_landing){
                    ...
                    delta_position_current = path[index]->p - position;
                    Sum_delta_position += delta_position_current;
                    double cmdX = Kp * delta_position_current.x() + Ki * Sum_delta_position.x() + Kd * (delta_position_current.x()-delta_position_previous.x()) ;
                    double cmdY = Kp * delta_position_current.y() + Ki * Sum_delta_position.y() + Kd * (delta_position_current.y()-delta_position_previous.y()) ;
                    double cmdZ = Kp * delta_position_current.z() + Ki * Sum_delta_position.z() + Kd * (delta_position_current.z()-delta_position_previous.z()) ;
                    cmd.linear.x = cmdX;
                    cmd.linear.y = cmdY;
                    cmd.linear.z = cmdZ;
                    move_pub.publish(cmd);
            ...
            //New step 
                ...
                if(distance(path[index-1]->p, path[index]->p) > step_len){
                    ...
                    index -= 1;
                    //Is it necessary, will have to be checked !
                    cmd.linear.x = 0;
                    cmd.linear.y = 0;
                    cmd.linear.z = 0;   
                }       
            ...
            //USUAL COMMAND
                ...
                double Kdbis = (index == (path.size()-1)) ? 0 : Kd;
                double cmdX  = Kp * delta_position_current.x() + Ki * Sum_delta_position.x() + Kdbis * (delta_position_current.x()-delta_position_previous.x());
                double cmdY  = Kp * delta_position_current.y() + Ki * Sum_delta_position.y() + Kdbis * (delta_position_current.y()-delta_position_previous.y());
                double cmdZ  = Kp * delta_position_current.z() + Ki * Sum_delta_position.z() + Kdbis * (delta_position_current.z()-delta_position_previous.z());
